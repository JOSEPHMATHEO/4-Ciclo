...
ALTER TABLE usuarios ADD CONSTRAINT chk_email CHECK (email LIKE '%@%' AND LENGTH(SUBSTR(email, 1, INSTR(email, '@') - 1)) > 1 AND LENGTH(SUBSTR(email, INSTR(email, '@') + 1)) > 1);

Alter Table prestamos ADD CONSTRAINT CHECK (fechadevolucion)
